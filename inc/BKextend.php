...
function place_special_pet_content( $content ) {
    ...
    $special .= '<div class="pet_info pet_'.get_the_id().'" >'.
                ...
                '<ul>'.
                test_if_meta( $petinfo, '_data_pet_gender', '<li><span>'.__('Gender','wp_pet').':</span> ', '</li>').
                test_if_meta( $petinfo, '_data_pet_size', '<li><span>'.__('Size','wp_pet').':</span> ', '</li>').
                test_if_meta( $petinfo, '_data_pet_age', '<li><span>'.__('Age','wp_pet').':</span> ', '</li>').
                '<li><span>'.__('Colors','wp_pet').'</span> '.get_the_term_list( $post->ID, 'pet-color', '', ', ', '' ).'</li>'.
                test_if_meta( $petinfo, '_data_pet_breed', '<li><span>'.__('Breed','wp_pet').':</span> ', '</li>').
                //'<li>'.$petinfo[_data_pet_fee][0].'</li>'.
                '</ul>'.

                '<ul>'.
                test_if_meta( $petinfo, '_data_pet_vaccines', '<li><span>'.__('Vaccines','wp_pet').':</span> ', '</li>').
                test_if_meta( $petinfo, '_data_pet_coat', '<li><span>'.__('Coat','wp_pet').':</span> ', '</li>').
                test_if_meta( $petinfo, '_data_pet_pattern', '<li><span>'.__('Pattern','wp_pet').':</span> ', '</li>').
                test_if_meta( $petinfo, '_data_pet_fee', '<li><span>'.__('Fee','wp_pet').':</span> ', '</li>').
                //'<li>'.$petinfo[_data_pet_fee][0].'</li>'.
                '</ul>'.
                '</div>';
                //test_if_meta( $petinfo, '_data_pet_extra', '<span>', '</span>');
...
function place_special_pet_content_in_pets( $content ) {
    ...
    if( is_page($page) ) {
      ...
      $data .= '<h3>'.__('Browse Pets by Status','wp_pet').'</h3><ul>'.wp_list_categories('echo=0&show_count=1&taxonomy=pet-status&title_li=').'</ul>'.
               ...
               '<h3>'.__('Browse Pets by Color','wp_pet').'</h3><ul>'.wp_list_categories('echo=0&show_count=1&taxonomy=pet-color&title_li=').'</ul>'.

               '<h3>'.__('Search Pets','wp_pet').'</h3><form method="get" id="pet_search" action="'. home_url() . '/">'.
               '<input type="hidden" name="post_type" value="pet" />'.
               '<select name="pet-status" id="drop-status">';
      ...
      $data .= '</select>&nbsp;<select name="pet-category" id="drop-type">';
            $terms = get_terms('pet-category', array('hide_empty' => 1 ));
            foreach ($terms as $term) {
      $data .= "<option value='$term->slug'" . selected($term->slug, true, false) . ">$term->name</option>";
        		 }
      $data .= '</select><input id="ada_search_submit" type="submit" value="'. __('Go','wp_pet') . '"/>'.
               '</form>';
